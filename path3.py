...
with open('Routemap-UttaraBus.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    for row in readCSV:
        lst = []
        i = 1
        ...
        for p in lst:
            list_of_point.append(p)
        while i < len(lst)-1:
            adj.add_edge(lst[i], lst[i+1], 'bus')
            graph.add_edge(lst[i], lst[i+1])
            i = i+1

with open('Routemap-BikolpoBus.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    for row in readCSV:
        lst = []
        i = 1
